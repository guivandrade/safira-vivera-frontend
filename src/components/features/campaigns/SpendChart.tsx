...
interface SpendChartProps {
  ...
  /** Mostra linha fantasma do período anterior (split do range atual em 2). */
...
function getValue(item: MonthlyData, platform: 'all' | 'meta' | 'google'): number {
  if (platform === 'meta') return item.meta.spend;
  if (platform === 'google') return item.google.spend;
  return item.totalSpend;
}

export function SpendChart({ data, platformFilter = 'all', onBarClick, showComparison = false }: SpendChartProps) {
      ...
      <ResponsiveContainer width="100%" height={260}>
        <ComposedChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
          ...
          <Tooltip
            ...
            content={<BreakdownTooltip platformFilter={platformFilter} showComparison={showComparison} format={formatCurrency} />}
          />
          <Bar
            dataKey="Total"
            fill={color}
...
function BreakdownTooltip({ active, payload, label, platformFilter, showComparison, format }: BreakdownTooltipProps) {
  ...
  return (
    <div className="rounded-md border border-line bg-surface px-3 py-2 text-xs shadow-md">
      ...
      {platformFilter === 'all' ? (
        <>
          <TooltipRow dotColor={META_COLOR} label="Meta" value={format(row.Meta)} />
          <TooltipRow dotColor={GOOGLE_COLOR} label="Google" value={format(row.Google)} />
          <div className="mt-1.5 border-t border-line-subtle pt-1.5">
            <TooltipRow label="Total" value={format(row.Total)} bold />
          </div>
        </>
      ) : (
        <TooltipRow
      ...
      {showComparison && prev !== undefined && (
        <div className="mt-1.5 border-t border-line-subtle pt-1.5">
          <TooltipRow label="Período anterior" value={format(prev)} muted />
        </div>
      )}
    </div>
  );
